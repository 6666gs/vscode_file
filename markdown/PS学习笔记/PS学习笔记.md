...
2. 魔棒工具
魔棒可以快捷地将颜色相同的区域变成选区
        ![](picture/2023-10-11-00-20-39.png)
        如图所示
    - 取样大小选取`取样点`，表示选取中比对的参数就是单个像素点的颜色像素
    - `容差`：`容差`越大，选取范围越大。即表示选取可以承受的差值越大
    - `消除锯齿`：即选区消除锯齿
    - `连续`：即选取颜色相同的且连续的区域为相同选区
        ![40%](picture/2023-10-11-00-24-54.png) ![40%](picture/2023-10-11-00-25-08.png)
        对比如图所示
    - 通过`魔棒`工具可以实现许多便捷高效的工作：
      - 例如，将图片中的两个小人从背景里面扣出来，因为背景都是绿色的，因此可以用魔棒点击绿色背景，之后选择反选(即右键点击，如图所示)
        ![60%](picture/2023-10-11-00-28-33.png)
        之后便可扣出两个小人
      - 而另一种操作实现效果与魔棒类似。即右键图像，点击`颜色范围`，则界面自动如下显示：![60%](picture/2023-10-11-00-33-37.png)
        利用吸管工具吸出绿色，即可同样扣出两个小人
3. 快速选取工具
魔棒工具较为简单，虽然神奇，但是其算法实现不够只能，仍然经常选错。因此需要快速选取工具
![](picture/2023-10-11-00-37-00.png)
快速选取工具与魔棒工具共用一个位置。
    - `自动增强`：识别边缘的能力会更强
   - 画笔选项
    ![30%](picture/2023-10-11-00-38-14.png)
        - `大小`：指画笔的识别大小
        - `硬度`：指画笔的边缘识别能力
        - `间距`：识别的连贯程度
    - 所谓画笔，这个工具就是像画画一样，把区域囊括进来的，如图
    ![60%](picture/2023-10-11-00-50-17.png)
    因此`画笔大小`其实越小，则画笔的细节识别能力越强
    - 对于如图这个下侧多出来的选区
    ![60%](picture/2023-10-11-00-51-37.png)
    可以按住`alt`键，采用相减模式，即可自动删去多余部分了
    - 注意，大选区选完后可能还得放大进行细节的选区
    ![60%](picture/2023-10-11-00-53-01.png)
    - 之后按下，`ctrl`+`j`就可以复制选中选区成为新的图层
